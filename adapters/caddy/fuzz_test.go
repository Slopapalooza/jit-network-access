...
//
// TestForgedXFFCannotChooseGrantKey checks that for one hand-written chain. This
// checks it for chains nobody wrote: repeated header lines, empty entries,
// bracketed and zoned IPv6, ports, whitespace, addresses inside the trusted range
// and junk that does not parse at all.
...
// Without trust_forwarded the peer is the only input, and NOTHING in a header may
...
func FuzzClientIPUntrustedIgnoresHeadersEntirely(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, xff, realIP string) {
		want, err := jitcore.CanonIP("203.0.113.9", j.IPv6Prefix, 32)
		if err != nil {
			t.Fatal(err)
		}
		r := mkreq(http.MethodGet, "/", "203.0.113.9:44000", nil)
		...
		if got != want {
			t.Fatalf("headers moved the grant key without trust_forwarded: "+
